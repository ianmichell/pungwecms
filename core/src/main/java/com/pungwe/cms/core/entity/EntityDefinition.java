...
public interface EntityDefinition<ET extends EntityTypeInfo> {
    ...
    default SortedSet<FieldConfig> getFieldsByGroup(String group) {
        ...
        return getFields().parallelStream().filter(f -> fieldGroup.get().getChildren().contains(f.getName()))
    ...
    default SortedSet<FieldGroupConfig> getFieldGroupsByParent(String group) {
        ...
        return getFieldGroups().parallelStream().filter(g -> fg.get().getChildren().contains(g.getName())).collect(
                Collectors.toCollection(TreeSet::new));
    }

    default Optional<FieldGroupConfig> getFieldGroupByName(String name) {
        return getFieldGroups() != null ? getFieldGroups().parallelStream().filter(g -> g.getName().equals(name))
                .findFirst() : Optional.empty();
    }

    default Optional<FieldConfig> getFieldByName(String name) {
        return getFields() != null ? getFields().parallelStream().filter(f -> f.getName().equals(name)).findFirst()
                : Optional.empty();
    }

    default SortedSet<FieldConfig> getFieldsWithoutParent() {
        if (getFields() == null) {
            return new TreeSet<>();
        }
        List<String> children = getFieldGroups().stream().flatMap(fieldGroupConfig -> fieldGroupConfig.getChildren()
                .stream()).collect(Collectors.toList());
        return getFields().stream().filter(fieldConfig -> !children.contains(fieldConfig.getName())).collect(
                Collectors.toCollection(TreeSet::new));
    }

    default SortedSet<FieldGroupConfig> getFieldGroupsWithoutParent() {
        if (getFieldGroups() == null) {
            return new TreeSet<>();
        }
        // Fetch a complete list of children to filter by.
        List<String> children = getFieldGroups().stream().flatMap(fieldGroupConfig -> fieldGroupConfig
                .getChildren().stream()).collect(Collectors.toList());
        // Return a sorted collection of FieldGroupConfig. If the group is found in children, then we exclude it.
        return getFieldGroups().stream().filter(fieldGroupConfig -> !children.contains(fieldGroupConfig.getName()))
                .collect(Collectors.toCollection(TreeSet::new));
    }
}
